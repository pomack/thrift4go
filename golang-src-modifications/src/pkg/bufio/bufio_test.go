...
func TestReadAndWriteByte(t *testing.T) {
  ...
  b := NewReadWriter(NewReader(buf), NewWriter(buf))
  b1 := []byte{123,'i',34,97,'h',62,92,40,'a'}
  for i:=1; i<=2; i++ {
    for k, v := range b1 {
      err := b.WriteByte(v)
      if err != nil {
        t.Errorf("ReadAndWriteByte pass %d error while writing byte %d at index %d: %q", i, v, k, err)
      }
    }
    b.Flush()
    n := buf.Len()
    if n != len(b1) {
      t.Errorf("ReadAndWriteByte pass %d expected len of %d after writing but received Len() of %d", i, len(b1), n)
    }
    for k, v := range b1 {
      c, err := b.ReadByte()
      if err != nil {
        t.Errorf("ReadAndWriteByte pass %d error while reading byte %d at index %d: %q", i, v, k, err)
      } else if c != v {
        t.Errorf("ReadAndWriteByte pass %d expected byte %d at index %d but received %d", i, v, k, c)
      }
    }
    c, err := b.ReadByte()
    if err != os.EOF {
      t.Errorf("ReadAndWriteByte pass %d expected os.EOF but received %q", i, err)
    }
    if c != 0 {
      t.Errorf("ReadAndWriteByte pass %d expected byte 0 with os.EOF but received %d", i, c)
    }
  }
}
...
func TestReadAndWrite(t *testing.T) {
  ...
  n, err := b.Write(b1);
  if err != nil {
    t.Error("ReadAndWrite", err)
  }
  if n != len(b1) {
    t.Errorf("ReadAndWrite 1st pass expected to write %d bytes but only wrote %d bytes", len(b2), n)
  }
  b.Flush()
  m := make([]byte, 0, 1000)
  n, err = b.Read(m)
  ...
  if n != len(b1) {
    t.Errorf("ReadAndWrite 1st pass expected read length %d got %d", len(b1), n)
  }
  for k, v := range m {
    if v != b1[k] {
      t.Errorf("ReadAndWrite 1st pass expected %q at index %d found %q", b1[k], k, v)
    }
  }
  n, err = b.Read(m)
  if err != os.EOF {
    t.Errorf("ReadAndWrite 2nd pass expected os.EOF got %q", err)
  }
  if n != 0 {
    t.Errorf("ReadAndWrite 2nd pass expected read length %d got %d", 0, n)
  }
  n, err = b.Write(b2)
  if err != nil {
    t.Error("ReadAndWrite", err)
  ...
  b.Flush()
  if n != len(b2) {
    t.Errorf("ReadAndWrite 2nd pass expected to write %d bytes but only wrote %d bytes", len(b2), n)
  }
  n, err = b.Read(m)
  if n != len(b2) {
    t.Errorf("ReadAndWrite 3rd pass expected read length %d got %d", len(b2), n)
  }
  for k, v := range m {
    if v != b2[k] {
      t.Errorf("ReadAndWrite 3rd pass expected %q at index %d found %q", b2[k], k, v)
    }
  }
}
