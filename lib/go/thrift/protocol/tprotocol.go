...
func Skip(self TProtocol, fieldType TType, maxDepth int) (err TProtocolException) {
    switch(fieldType) {
      case STOP: return;
      case BOOL: _, err = self.ReadBool(); return;
      case BYTE: _, err =  self.ReadByte(); return;
      case I16: _, err = self.ReadI16(); return;
      case I32: _, err = self.ReadI32(); return;
      case I64: _, err = self.ReadI64(); return;
      case DOUBLE: _, err = self.ReadDouble(); return;
      case STRING: _, err = self.ReadString(); return;
      case STRUCT: {
