...
func (p *context) WriteBinary(value []byte) os.Error {
  if p.writer == nil { return os.EOF }
  p.value = value
  if _, e := p.writer.Write(JSON_QUOTE_BYTES); e != nil { return e }
  writer := base64.NewEncoder(base64.StdEncoding, p.writer)
  if _, e := writer.Write(value); e != nil { return e }
  if e := writer.Close(); e != nil { return e }
  if _, e := p.writer.Write(JSON_QUOTE_BYTES); e != nil { return e }
  return nil
}
...
func (p *context) Populate(r *bufio.Reader) os.Error {
  ...
  case JSON_LBRACKET[0]:
    cxt := NewListContext();
    e = cxt.Populate(r);
    if e != nil { return NewTProtocolExceptionFromOsError(e); }
    p.value = cxt;
  case JSON_LBRACE[0]:
...
func (p *ListContext) WriteNull() os.Error {
  if p.writer == nil { return os.EOF }
  if e := p.writeContext(); e != nil { return e }
  _, err := p.writer.Write(JSON_NULL)
  return err
}
...
func (p *ListContext) WriteBinary(value []byte) os.Error {
  if p.writer == nil { return os.EOF }
  if e := p.writeContext(); e != nil { return NewTProtocolExceptionFromOsError(e) }
  if _, e := p.writer.Write(JSON_QUOTE_BYTES); e != nil { return NewTProtocolExceptionFromOsError(e) }
  writer := base64.NewEncoder(base64.StdEncoding, p.writer)
  if _, e := writer.Write(value); e != nil { return NewTProtocolExceptionFromOsError(e) }
...
func (p *ListContext) Close() os.Error {
  if p.first { if e := p.writeContext(); e != nil { return e } }
...
func (p *ObjectContext) WriteBool(value bool) os.Error {
  ...
  if p.onKey {
    if e := p.writeContext(); e != nil { return e }
    if _, e := p.writer.Write(v); e != nil { return e }
    p.onKey, p.onValue = p.onValue, p.onKey
    return nil
  }
  if e := p.writeContext(); e != nil { return e }
...
func (p *ObjectContext) writeContext() os.Error {
  ...
  if !p.keyType.IsBaseType() {
    v := JSON_COMMA
    if p.onValue { v = json_nonbase_map_elem_bytes }
    _, err := p.writer.Write(v)
    return err
  }
  v := JSON_COMMA
  if p.onValue { v = JSON_COLON }
  _, err := p.writer.Write(v)
  return err
}
...
func (p *ObjectContext) Close() os.Error {
  ...
  if p.onValue {
    if e := p.WriteNull(); e != nil { return e }
  }
  if !p.keyType.IsBaseType() {
...
func (p *TSimpleJSONProtocol) WriteMessageBegin(name string, typeId TMessageType, seqId int32) TProtocolException {
  if e := p.writeContext.WriteStartSubContext(); e != nil { return NewTProtocolExceptionFromOsError(e) }
  p.pushWriteContext(NewListContextETypeWriter(TType(VOID), p.writer));
  if e := p.WriteString(name); e != nil { return e }
  if e := p.WriteByte(byte(typeId)); e != nil { return e }
  if e := p.WriteI32(seqId); e != nil { return e }
  return nil
}

func (p *TSimpleJSONProtocol) WriteMessageEnd() TProtocolException {
  if e := p.popWriteContext().Close(); e != nil { return NewTProtocolExceptionFromOsError(e) }
...
func (p *TSimpleJSONProtocol) ReadMessageBegin() (name string, typeId TMessageType, seqId int32, err TProtocolException) {
  ...
  if e != nil { err = NewTProtocolExceptionFromOsError(e); return; }
...
func (p *TSimpleJSONProtocol) ReadMapBegin() (keyType TType, valueType TType, size int, e TProtocolException) {
  ...
  if err != nil { return STOP, STOP, 0, NewTProtocolExceptionFromOsError(err); }
  var b byte;
...
func (p *TSimpleJSONProtocol) ReadListBegin() (elemType TType, size int, e TProtocolException) {
  err := p.readContext.Read(p.reader);
  if err != nil { return STOP, 0, NewTProtocolExceptionFromOsError(err); }
  var b byte;
  b, err = p.reader.ReadByte();
  if err != nil { return STOP, 0, NewTProtocolExceptionFromOsError(err); }
...
func (p *TSimpleJSONProtocol) ReadBool() (bool, TProtocolException) {
  ...
  err := p.readContext.Read(p.reader);
  if err != nil { return false, NewTProtocolExceptionFromOsError(err); }
  c, e := p.reader.ReadByte();
  if e != nil { return false, NewTProtocolExceptionFromOsError(e); }
  var b []byte;
  var value bool;
  switch c {
  case JSON_TRUE[0]:
...
func readNumeric(reader *bufio.Reader) (Numeric, TProtocolException) {
  ...
  if isNull || err != nil { return NUMERIC_NULL, err; }
  ...
  for continueFor {
    ...
    case JSON_NAN[0]:
      if buf.Len() == 0 {
        ...
        if e != nil { return NUMERIC_NULL, NewTProtocolExceptionFromOsError(e) }
...
func readBase64EncodedBody(reader *bufio.Reader) ([]byte, TProtocolException) {
  ...
  if err != nil { return line, NewTProtocolExceptionFromOsError(err); }
...
func (p *TSimpleJSONProtocol) ReadBinary() ([]byte, TProtocolException) {
  ...
  p.readContext.Read(p.reader)
  _, err := p.reader.ReadString(JSON_QUOTE);
  if err != nil { return []byte{}, NewTProtocolExceptionFromOsError(err); }
