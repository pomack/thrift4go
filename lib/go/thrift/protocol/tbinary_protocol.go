...
func (p *TBinaryProtocol) ReadMessageBegin() (name string, typeId TMessageType, seqId int32, err TProtocolException) {
  size, e := p.ReadI32();
  if e != nil { return "", typeId, 0, NewTProtocolExceptionFromOsError(e); }
  if size < 0 {
    ...
    name, e = p.ReadString();
    if e != nil { return name, typeId, seqId, NewTProtocolExceptionFromOsError(e); }
    seqId, e = p.ReadI32();
    if e != nil { return name, typeId, seqId, NewTProtocolExceptionFromOsError(e); }
  ...
  if e2 != nil { return name, typeId, seqId, e2; }
  b, e3 := p.ReadByte();
  if e3 != nil { return name, typeId, seqId, e3; }
  typeId = TMessageType(b);
  seqId, e4 := p.ReadI32();
  if e4 != nil { return name, typeId, seqId, e4; }
  return name, typeId, seqId, nil;
}
...
func (p *TBinaryProtocol) ReadFieldBegin() (name string, typeId TType, seqId int16, err TProtocolException) {
  t, err := p.ReadByte();
  typeId = TType(t);
  if err != nil { return name, typeId, seqId, err; }
  if t != STOP {
    seqId, err = p.ReadI16();
  }
