...
func NewNumericFromDouble(dValue float64) Numeric {
  if math.IsInf(dValue, 1) { return INFINITY }
  if math.IsInf(dValue, -1) { return NEGATIVE_INFINITY }
  if math.IsNaN(dValue) { return NAN }
  iValue := int64(dValue);
...
func NewNumericFromString(sValue string) Numeric {
  if sValue == INFINITY.String() { return INFINITY }
  if sValue == NEGATIVE_INFINITY.String() { return NEGATIVE_INFINITY }
  if sValue == NAN.String() { return NAN }
...
func NewNumericFromJSONString(sValue string, isNull bool) Numeric {
  if isNull { return NewNullNumeric() }
  if sValue == JSON_INFINITY { return INFINITY }
  if sValue == JSON_NEGATIVE_INFINITY { return NEGATIVE_INFINITY }
  if sValue == JSON_NAN { return NAN }
  iValue, _ := strconv.Atoi64(sValue);
  dValue, _ := strconv.Atof64(sValue);
