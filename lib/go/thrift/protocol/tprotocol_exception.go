...
func NewTProtocolExceptionFromOsError(e os.Error) TProtocolException {
  if e == nil { return nil }
  if t, ok := e.(TProtocolException); ok { return t }
  if te, ok := e.(transport.TTransportException); ok { return NewTProtocolExceptionFromTransportException(te) }
  if _, ok := e.(base64.CorruptInputError); ok { return NewTProtocolException(INVALID_DATA, e.String()) }
