...
func (p *tField) CompareTo(other interface{}) (int, bool) {
  ...
  if data, ok := other.(TField); ok {
    if p.Id() != data.Id() { return CompareInt16(p.Id(), data.Id()), true }
    if p.TypeId() != data.TypeId() { return CompareByte(byte(p.TypeId()), byte(data.TypeId())), true }
...
func (p *tFieldContainer) At(i int16) TField {
  ...
  for _, field := range p.fields {
    if i == field.Id() { return field }
    if i > field.Id() { return nil }
  }
  return nil
}
...
func (p *tFieldContainer) Contains(data interface{}) bool {
  if i, ok := data.(int); ok {
    for _, field := range p.fields {
      if field.Id() == int16(i) { return true }
    }
  } else if i, ok := data.(int16); ok {
    for _, field := range p.fields {
      if field.Id() == i { return true }
    }
  } else if s, ok := data.(string); ok {
    for _, field := range p.fields {
      if field.Name() == s { return true }
    }
...
func (p *tFieldContainer) Equals(other interface{}) bool {
  ...
  if data, ok := other.(TFieldContainer); ok {
    if p.Len() != data.Len() { return false }
    for _, field := range p.fields {
...
func (p *tFieldContainer) CompareTo(other interface{}) (int, bool) {
  ...
  if data, ok := other.(TFieldContainer); ok {
    ...
    if ok2 && p == cont { return 0, true }
    if cmp := CompareInt(p.Len(), data.Len()); cmp != 0 { return cmp, true }
    for _, field := range p.fields {
      if cmp, ok3 := field.CompareTo(data.At(field.Id())); !ok3 || cmp != 0 { return cmp, ok3 }
    }
    return 0, true
  }
  return 0, false
