...
func (p *tMap) Get(key interface{}) (interface{}, bool) {
  ...
  case STRUCT:
    for elem := range p.l.Iter() {
      ...
      if reflect.DeepEqual(useKey, k) { return e.Value(), true }
    }
    return nil, false
  case MAP:
    for elem := range p.l.Iter() {
...
func (p *tMap) Set(key, value interface{}) {
  ...
  case STOP, VOID:
    ...
    return
  case BOOL:
    if p.b == nil { p.b = make(map[bool]interface{}) }
    b := coercedKey.(bool)
    p.b[b] = value
  case BYTE:
    if p.i08 == nil { p.i08 = make(map[byte]interface{}) }
    b := coercedKey.(byte)
    p.i08[b] = value
  case DOUBLE:
    ...
    p.f64[b] = value
  case I16:
    if p.i16 == nil { p.i16 = make(map[int16]interface{}) }
    b := coercedKey.(int16)
    p.i16[b] = value
  case I32:
    if p.i32 == nil { p.i32 = make(map[int32]interface{}) }
    b := coercedKey.(int32)
    p.i32[b] = value
  case I64:
    if p.i64 == nil { p.i64 = make(map[int64]interface{}) }
    b := coercedKey.(int64)
    p.i64[b] = value
  case STRING, UTF8, UTF16:
    if p.s == nil { p.s = make(map[string]interface{}) }
    b := coercedKey.(string)
    p.s[b] = value
...
func (p *tMap) Contains(key interface{}) bool {
  ...
  case STRUCT:
    for elem := range p.l.Iter() {
      ...
      if reflect.DeepEqual(coercedKey, k) { return true }
    }
    return false
  case MAP:
    for elem := range p.l.Iter() {
